...
# Base environnement
        ...
        cd byterub/utils/build_scripts
        docker build -f android32.Dockerfile -t byterub-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t byterub-gui-android .
        docker create -it --name byterub-gui-android byterub-gui-android bash

# Get the apk

        docker cp byterub-gui-android:/opt/android/byterub-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/byterub-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
