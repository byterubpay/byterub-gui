...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = byterubAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date, _nettype){
    // time of byterub birth 2014-04-18 10:49:53 (1397818193)
    var byterubBirthTime = _nettype == "Mainnet" ? 1397818193 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    ...
    var secondsPerBlock;
    // before byterub's birth
    if (requestedTime < byterubBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < byterubBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > byterubBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - byterubBirthTime)/secondsPerBlockV1);
