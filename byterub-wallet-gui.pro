...
# cleaning "auto-generated" bitbyterub directory on "make distclean"
...
linux {
   ...
   #     }
    ...
    # currently byterub has an issue with "static" build and linunwind-dev,
...
# translation stuff
TRANSLATIONS = $$files($$PWD/translations/byterub-core_*.ts)
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libbyterub-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
...
DISTFILES += \
    notes.txt \
    byterub/src/wallet/CMakeLists.txt
