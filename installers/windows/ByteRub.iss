; ByteRub Carbon Chamaeleon GUI Wallet Installer for Windows
; Copyright (c) 2017-2019, The ByteRub Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\byterub-wallet-gui.exe")
...
LicenseFile=LICENSE
AppPublisher=The ByteRub Developer Community
AppPublisherURL=https://getbyterub.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the ByteRub files can be copied to a place where they have at least
...
; As of version 0.15.0.0, the ByteRub .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to ByteRub.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; ByteRub GUI wallet exe and guide
Source: "bin\byterub-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; ByteRub CLI wallet
Source: "bin\byterub-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; ByteRub wallet RPC interface implementation
Source: "bin\byterub-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; ByteRub daemon
Source: "bin\byterubd.exe"; DestDir: "{app}"; Flags: ignoreversion

; ByteRub daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "byterub-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; ByteRub blockchain utilities
...
Source: "bin\byterub-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitbyterub');
  s := 'The default folder to store the ByteRub blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\byterubpay\byterub-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" ByteRub default location
...
begin
  Result := BlockChainDir('') + '\bitbyterub.log';
...
begin
  if CurStep = ssPostInstall then begin
    ...
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the ByteRub daemon and then stay with window open after it exits';
...
[Icons]
; Icons in the "ByteRub GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\byterub-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\byterub-gui-wallet-guide.pdf"; IconFilename: "{app}\byterub-wallet-gui.exe"
...
Name: "{group}\Utilities\ByteRub Daemon"; Filename: "{app}\byterubd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\byterub-wallet-cli.exe"; WorkingDir: "{userdocs}\ByteRub\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\ByteRub\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\byterub-wallet-gui\byterub-wallet-gui.log"""
...
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\byterub-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\byterubpay"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\byterubpay\byterub-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\byterubpay\byterub-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "byterub:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "byterub://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "byterub"; ValueType: "string"; ValueData: "URL:ByteRub Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "byterub"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "byterub\DefaultIcon"; ValueType: "string"; ValueData: "{app}\byterub-wallet-gui.exe,0"
Root: HKCR; Subkey: "byterub\shell\open\command"; ValueType: "string"; ValueData: """{app}\byterub-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "byterubseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "byterubseed://a.b.c.d:port"
Root: HKCR; Subkey: "byterubseed"; ValueType: "string"; ValueData: "URL:ByteRub Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "byterubseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "byterubseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\byterub-wallet-gui.exe,0"
Root: HKCR; Subkey: "byterubseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\byterub-wallet-gui.exe"" ""%1"""
